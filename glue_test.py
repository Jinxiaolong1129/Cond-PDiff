...
# def GLUE_test(model_args, data_args, training_args, param, checkpoints_dir, positions, original_shapes):
...
class DataTrainingArguments:
    ...
    gpu_list: Optional[str] = field(
        default="0",
        metadata={"help": "gpu list"},
    )
...
def args_GLUE_test(config_path):
...
def args_multiple_GLUE_test(config_path, dataset_name, model, rank):
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments))
    if config_path.endswith(".json"):
        config = json.load(open(config_path, 'r'))
        dataset_config = config[model][str(rank)][dataset_name]
        model_args, data_args, training_args = parser.parse_dict(dataset_config)
...
def GLUE_test(model_args, data_args, training_args, param, checkpoints_dir, positions, original_shapes):
    ...
    # logging.getLogger("transformers").setLevel(logging.ERROR)
    # logging.getLogger("datasets").setLevel(logging.ERROR)
    # logging.getLogger("evaluate").setLevel(logging.ERROR)  # If you're using `evaluate` library
    # logging.getLogger("peft").setLevel(logging.ERROR)  # If using `peft`
    ...
    def update_model_adapter_weight(model, state_dict,adapter_name='default'):
    ...
    state_dict = get_update_adapter_weight(param, positions, original_shapes)
    model = PeftModel.from_pretrained(model = model, model_id = checkpoints_dir)   
    peft_model = update_model_adapter_weight(model, state_dict, adapter_name='default')
...
def GLUE_test_inference(model_args, data_args, training_args, param,checkpoints_dir, positions, original_shapes):
    if not os.path.exists(training_args.logging_dir):
        os.makedirs(training_args.logging_dir)
