...
class DDPM(nn.Module):
    ...
    def generate(self, batch, cond = None, num=10, history=False):
        model = self.model.ema if hasattr(self.model, 'ema') else self.model
        model.eval()
        shape = (num, 1, batch.shape[1] * batch.shape[2]) # condition
        sample = self.progressive_samples_fn_simple( 
            model,
            shape,
            device=self.device_diffusion,
            cond = cond,
    ...
    def progressive_samples_fn_simple(self, model, shape, device, cond,  include_x0_pred_freq=50, history=False):
    ...
    def validation_epoch(self, batch, batch_idx, **kwargs: Any):
        batch = self.pre_process(batch)
        outputs = self.generate(batch, 10)

        params = self.post_process(outputs)
        params = params.cpu()
        # test diff model generated para acc on nlp tasks
        accs = []
    ...
    def forward(self, batch, condition, **kwargs):
        batch = self.pre_process(batch)
        model = self.model # ema model and model 
        ...
        if self.loss_type == 'kl':
            ...
            model_output = model(x_t, time, condition=condition)
    ...
    def p_mean_variance(self, model, x, t, clip_denoised, return_pred_x0, cond=None):
        ...
        if cond is not None:
            model_output = model(x, t, condition=cond)
        else:
    ...
    def p_sample(self, model, x, t, noise_fn, clip_denoised=True, return_pred_x0=False, cond=None):
        ...
        del mean, log_var, noise, nonzero_mask
    ...
    def p_sample_loop_progressive_simple(self, model, shape, device, noise_fn=torch.randn,
                                         include_x0_pred_freq=50,input_pa=None,exp_step=None, cond=None):
