...
class AE_CNN_bottleneck(nn.Module):
    ...
    def forward(self, input, time, cond=None):
        ...
        input_shape = input.shape
        input = input.reshape(input.shape[0], 1, -1) # [10 1 12]
        # import pdb;pdb.set_trace()
        time_info = self.time_encode(time)[0, None, None]
        time_info = time_info.repeat((input.shape[0], 1, 1)) # repeat 10 times / [10 1 12]

        if len(input.size()) == 2:
            input = input.view(input.size(0), 1, -1)

        input = input # B x 1 x in_dim
        ...
        emb_dec1 = self.dec1(emb_enc4 + time_info) + emb_enc3
        emb_dec2 = self.dec2(emb_dec1 + time_info) + emb_enc2 # u-net ? TODO 为什么加入emb_enc2
        emb_dec3 = self.dec3(emb_dec2 + time_info) + emb_enc1
        emb_dec4 = self.dec4(emb_dec3 + time_info)
        # import pdb; pdb.set_trace()
...
class AE_CNN_bottleneck_cond(nn.Module):
    def __init__(
            self,
            in_dim,
            in_channel,
            time_step=1000,
            num_conditions=2,  # Number of different conditions
            cond_emb_size=1024,  # Size of the condition embedding
            dec=None
    ):
        super().__init__()
        ...
        if num_conditions > 0:
            self.cond_embedding = nn.Embedding(num_conditions, cond_emb_size)
            self.use_conditions = True
        else:
            self.use_conditions = False

        self.real_input_dim = in_dim + cond_emb_size
        ...
        self.final_conv = nn.Linear(self.real_input_dim, self.in_dim)
    ...
    def forward(self, input, time, condition=None):
        ...
        if condition is not None:
            cond_emb = self.cond_embedding(condition).unsqueeze(1)
            input = torch.cat((input, cond_emb), dim=2)  # Now input is [B, 1, cond_emb_size + self.in_dim]
        ...
        final_output = self.final_conv(emb_dec4)
        final_output = final_output.reshape(input_shape)
...
class AE_CNN_bottleneck_cond_clip(nn.Module):
    def __init__(
            self,
            in_dim,
            in_channel,
    ...
    def forward(self, input, time, condition=None):
        ...
        clip_text_embedding = condition
        if clip_text_embedding is not None:
            clip_text_embedding = clip_text_embedding.unsqueeze(1)
            input = torch.cat((input, clip_text_embedding), dim=2)  # Concatenate CLIP text embedding with input
